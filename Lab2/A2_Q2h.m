...
%% BER Analysis
SNR_space = linspace(-10,50,200); %Most BER curves usually plot from SNRs of -10 dB to 50 dB
BER_viterbi_AWGN = zeros(200,1);

for i = 1:200
    n_err_viterbi_AWGN = zeros(10000,1); %Sending 10 000 packets
    for j = 1:10000
        [word, c] = viterbi_decod(SNR_space(i),G);
        n_err_viterbi_AWGN(j) =  sum(abs(c-word));
    end
    BER_viterbi_AWGN(i) = sum(n_err_viterbi_AWGN)/80000;
end

semilogy(SNR_space,BER_viterbi_AWGN);
xlabel('SNR (dB)');
ylabel('BER');
title('AWGN BER analysis; 8,4,4 Viterbi decoder');

%% decoder function

function [word, c] = viterbi_decod(SNR,G)
    p1 = 0.5;
    m = rand(1,4)<p1; %generate message vector
    c = mod(m*G,2); %generate codeword c = mG
    temp1 = c(4); %need to swap m4 and p1 bits since that is how the Viterbi trellis encodes (8,4,4) Hamming
    c(4) = c(5); c(5) = temp1;
    var = 0.5/(10^(SNR/10));
    n = var*randn(1,8);
    r = c+n;
    ...
    %% Add-store-compare path lengths of second stage
    %path_000
    p_000=zeros(1,2);
    d_00 = r(3:4)-[0 0];
